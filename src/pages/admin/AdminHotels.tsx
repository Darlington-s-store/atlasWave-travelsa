...
const AdminHotels = () => {
  ...
  const fetchBookings = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("bookings").select("*").eq("type", "hotel").order("created_at", { ascending: false });
  ...
  const filtered = bookings.filter(b => {
    const matchesSearch = b.route.toLowerCase().includes(search.toLowerCase());
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          ...
          <div className="text-[12px] text-muted-foreground bg-muted/40 px-3 py-1.5 rounded-lg border border-border/60">
            Bookings created by guests appear here for management.
          </div>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Total Hotels", value: bookings.length, icon: Hotel, bg: "bg-primary/10", color: "text-primary" },
          ...
          <CardContent className="p-4">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <div className="relative flex-1 max-w-sm">
                ...
                <Input placeholder="Search by hotel or location..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 h-9 text-[13px]" />
              </div>
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-[15px] font-semibold text-foreground">No hotel bookings yet</p>
                <p className="text-[13px] text-muted-foreground mt-1">Reservations made on the website will appear here for you to confirm, edit, or cancel.</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/30 hover:bg-muted/30">
                    <TableHead className="text-[11px] uppercase tracking-wider font-bold">Hotel / Location</TableHead>
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          {viewingBooking && (
            <div className="grid grid-cols-2 gap-3 py-2 text-[13px]">
              <div><span className="text-muted-foreground block text-[11px] uppercase tracking-wider font-bold mb-0.5">Hotel / Location</span>{viewingBooking.route}</div>
