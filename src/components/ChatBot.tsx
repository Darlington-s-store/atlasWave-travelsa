...
const ChatBot = () => {
  ...
  // Reliable WebAudio ping
  const playNotify = useCallback(() => {
    playPing(0.2);
  }, []);

  // Strip raw slash-route mentions from bot text (e.g. /visa-assistance, `/contact`, "/travel/flights")
  const sanitizeBotText = useCallback((text: string): string => {
    if (!text) return text;
    // Remove backtick-wrapped paths first: `/something`
    let cleaned = text.replace(/`\/[a-z0-9][a-z0-9/_-]*`/gi, "");
    // Remove markdown links pointing to internal paths: [label](/path) -> **label**
    cleaned = cleaned.replace(/\[([^\]]+)\]\(\/[a-z0-9][a-z0-9/_-]*\)/gi, "**$1**");
    // Remove plain slash routes like /visa-assistance, /travel/flights/123
    cleaned = cleaned.replace(/(^|[\s(])\/[a-z][a-z0-9/_-]{2,}/gi, "$1");
    // Tidy double spaces and stray empty parens
    ...
    try {
      ...
      if (!notifiedAdminThisSession.current) {
        ...
        createNotification({
          ...
          body: `${who}: ${(trimmed || "[attachment]").slice(0, 140)}`,
          type: "chat",
          link: "/admin/chatbot",
